...
export default function FloatingToothCard() {
    ...
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            // Initial drop-down animation when page loads
    ...
    return (
        <div
            ref={containerRef}
            className="fixed top-1/2 right-[10%] -translate-y-1/2 z-40"
            style={{ perspective: "1000px" }}
        >
            {/* Hover wrapper - handles scale/lift effects */}
            <div
                className="cursor-pointer"
                style={{
                    transform: isHovered ? "scale(1.05) translateY(-10px)" : "scale(1) translateY(0)",
                    transition: "transform 0.4s cubic-bezier(0.34, 1.56, 0.64, 1), filter 0.3s ease",
                    filter: isHovered ? "drop-shadow(0 25px 50px rgba(99, 102, 241, 0.3))" : "drop-shadow(0 10px 30px rgba(0, 0, 0, 0.15))"
                }}
                onMouseEnter={() => setIsHovered(true)}
                onMouseLeave={() => setIsHovered(false)}
            >
                {/* Flip card - handles rotation only */}
                <div
                    ref={cardRef}
                    className="relative w-[260px] h-[340px] md:w-[300px] md:h-[400px]"
                    style={{
